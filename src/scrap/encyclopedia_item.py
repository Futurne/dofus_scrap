...
class ScrapItem:
    ...
    def valid_container(element: WebElement) -> bool:
        ...
        for child in element.find_elements(By.XPATH, "./*"):
            if TITLE_CLASS in child.get_attribute("class"):
                ...
                title_count += int(
                    any(title.startswith(v) for v in VALID_CONTAINER_TITLES)
    ...
    def scrap_container(element: WebElement) -> Union[str, list[str], list[list[str]]]:
            ...
            for el in element.find_elements(By.CLASS_NAME, ITEM_LIST_CLASS):
                try:
                    ...
                    e = (
                        e.text
                        if e.text != ""
                        else e.get_attribute("textContent").strip()
                    )
                    ...
                    if len(el.find_elements(By.CLASS_NAME, "ak-text")) != 0:
                        e += " | " + " | ".join(
                            [
                                t.text.strip()
                                for t in el.find_elements(By.CLASS_NAME, "ak-text")
                            ]
                        )
                    ...
                    if len(el.find_elements(By.CLASS_NAME, "ak-front")) != 0:
                        e += " | " + " | ".join(
                            [
                                t.text.strip()
                                for t in el.find_elements(By.CLASS_NAME, "ak-front")
                            ]
                        )
        ...
        if (
            len(element.find_elements(By.CLASS_NAME, LIST_CLASS)) == 1
        ):  # List of elements
            return scrap_list(element.find_element(By.CLASS_NAME, LIST_CLASS))
        elif (
            len(element.find_elements(By.CLASS_NAME, LIST_CLASS)) != 0
        ):  # Multiple lists of elements
            return [
