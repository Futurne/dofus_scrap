...
class EncyclopediaScrap:
    ...
    def check_for_cookies_panel(self) -> bool:
        ...
        try:
            element = self.driver.find_element(
                By.CLASS_NAME, "ak-block-cookies-infobox"
            )
        ...
        raise RuntimeError(
            "Cookies panel has been found but no there is no accept button."
        )
    ...
    def list_items(self, url: str) -> list[str]:
        ...
        items = [tr.find_element(By.TAG_NAME, "a").get_attribute("href") for tr in trs]
        return items
    ...
    def total_pages(self, url: str) -> int:
        ...
        pages = [int(l.text) for l in lis if l.text.isdecimal()]
        return max(pages)
        ...
        if os.path.exists("data_raw/encyclopedia_state.yaml"):
            with open("data_raw/encyclopedia_state.yaml") as state_file:
                state |= yaml.safe_load(
                    state_file
                )  # Get the dictionnary and make it a defaultdict (thanks to the |= operator)
