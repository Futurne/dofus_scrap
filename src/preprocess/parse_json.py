...
BUFF_REGEX = "|".join(b.replace("(", r"\(").replace(")", r"\)") for b in VALID_BUFFS)
BUFF_REGEX += (
    r"|Dommage\(s\)(\s("
    + "|".join(chain(ELEMENTS, ["Poussée", "Critiques", "Pièges"]))
    + "))?"
...
DMG_REGEX = (
    "(" + "|".join(b.replace("(", r"\(").replace(")", r"\)") for b in VALID_DMGS) + ")"
)
...
class JsonParser:
    ...
    def parse_croisements(self, name: str, croisements: str):
        ...
        croisements = [
            (croisements[i], croisements[i + 1]) for i in range(0, len(croisements), 2)
        ]
    ...
    def parse_butins(self, name: str, butins: list[Union[list[str], str]]):
        def parse(
            butins: Union[str, list[str]]
        ) -> Union[dict[str, float], dict[str, tuple[float, float]]]:
    ...
    def parse_conditions_recursively(
        ...
        match conditions:
            case str():
                ...
                return {bool_op: (left_cond, int(right_cond))}
            case dict():
                return {
            ...
            case list():
                return [JsonParser.parse_conditions_recursively(c) for c in conditions]
            case None:
                return None
            case _:
                ...
                match cond:
                    ...
                    case list():
                        ...
                        if sub_conds != []:
                            ...
                            parsed.append({sub_bool_op: sub_conds})
